...
function createAdBanner() {
  ...
  function subscribe(observerFunction: Function): void {
    ...
    return (
      <div className="w-full rounded-[0.8rem] pt-[0.4rem] bg-nlw-border">
        <div className="flex justify-between items-center rounded-[0.8rem] py-[2.4rem] px-[3.2rem] bg-[#2A2634]">
          ...
          <button onClick={notifyAll} className="font-medium text-white flex gap-[1.2rem] rounded-[0.6rem] py-[1.2rem] px-[1.6rem] bg-violet-500 hover:bg-violet-600 duration-300">
            <img src={searchIcon}/>
            Publicar anúncio
          </button>
        </div>
